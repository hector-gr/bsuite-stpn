...
from bsuite.baselines.jax import actor_critic_stpn
...
def run(bsuite_id: str) -> str:
  ...
  agent = actor_critic_stpn.default_agent(
...
def main(_):
  ...
  if bsuite_id in sweep.SWEEP:
    ...
    num_parallel_processes = (jax.device_count('gpu') or None)
    bsuite_sweep = getattr(sweep, bsuite_id)
    print(f'Running sweep over bsuite_id in sweep.{bsuite_sweep} over {num_parallel_processes} GPUs in parallel.')
    FLAGS.verbose = False
    pool.map_mpi(run, bsuite_sweep, num_processes=num_parallel_processes)
