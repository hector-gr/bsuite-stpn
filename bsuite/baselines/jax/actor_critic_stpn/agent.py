...
# ============================================================================
"""A STPN recurrent actor-critic agent implemented in JAX + Haiku."""

from typing import Any, Callable, NamedTuple, Tuple

from bsuite.baselines import base
from bsuite.baselines.utils import sequence
from bsuite.baselines.jax.actor_critic_stpn.stpn import STPNState, STPN
...
Value = jnp.ndarray
STPNStateT = Any
RecurrentPolicyValueNet = Callable[[jnp.ndarray, STPNStateT],
                                   Tuple[Tuple[Logits, Value], STPNStateT]]


class AgentState(NamedTuple):
  """Holds the network parameters, optimizer state, and STPN state."""
  params: hk.Params
  opt_state: Any
  rnn_state: STPNStateT
  rnn_unroll_state: STPNStateT


class ActorCriticSTPN(base.Agent):
  ...
  def __init__(
    ...
    def loss(trajectory: sequence.Trajectory, rnn_unroll_state: STPNStateT):
      """"Actor-critic loss."""
...
def default_agent(obs_spec: specs.Array,
  ...
  hidden_size = 256
  initial_rnn_state = STPNState(
      hidden=jnp.zeros((1, hidden_size), dtype=jnp.float32),
      synaptic=jnp.zeros((1, hidden_size, 2*hidden_size), dtype=jnp.float32) # Given embedding layer, we know input size is hidden_size.
  )

  def network(inputs: jnp.ndarray,
              state) -> Tuple[Tuple[Logits, Value], STPNStateT]:
    flat_inputs = hk.Flatten()(inputs)
    torso = hk.nets.MLP([hidden_size, hidden_size])
    stpn = STPN(hidden_size)
    policy_head = hk.Linear(action_spec.num_values)
    value_head = hk.Linear(1)

    embedding = torso(flat_inputs)
    embedding, state = stpn(embedding, state)
    logits = policy_head(embedding)
    value = value_head(embedding)
    return (logits, jnp.squeeze(value, axis=-1)), state

  return ActorCriticSTPN(
      obs_spec=obs_spec,
